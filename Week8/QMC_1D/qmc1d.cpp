...
double external_potential(double val)
{
	...
	return pow(val,4)-2.5*val*val; //x^4 - 5/2 x^2
}
...
double external_potential_prime(double val)
{
	...
	return 4*pow(val,3) - 5.*val; // 4x^3 -5x
}
...
double variationalWaveFunction(double v)
{
	...
	double exp_m = exp(-(v-mu)*(v-mu)/(2.*sigma*sigma));
	double exp_p = exp(-(v+mu)*(v+mu)/(2.*sigma*sigma));
...
double variationalWaveFunction_second(double v)
{
	...
	return (pow(v-mu,2)-sigma*sigma)/pow(sigma,4) * exp_m  + (pow(v+mu,2)-sigma*sigma)/pow(sigma,4) * exp_p; 
