...
void Population::OutPrint(){
	ofstream Path_out("out.path.final");
...
void Path::OutFinal(){
	...
	for(int i=0;i < size; i++){
		//Path_out <<  paths[0].path[i] << "   ";
		Path_out << pool->cities[path[i]-1].x << "   " << pool->cities[path[i]-1].y << endl;
...
void Path::OutLength(){
	...
	ofstream Length_out("out.best.paths",ios::app);
...
double Path::Boltzmann(double T){
