...
{ 
  ...
  int nconf = 1;
  //ofstream Ene_out("energy.out");
  //ofstream Press_out("pressure.out");
  for(int ieq=0; ieq < 500; ieq++){  //some measureless steps for equilibration
	Move();
  }
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();      
      //Ene_out << walker[iv]/(double)npart + vtail << endl;				//print instantaneus values
      //Press_out << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
...
// distance ip-i in pbc
      ...
      if(dr < rcut)
      {
        ene += 1.0/pow(dr,12) - 1.0/pow(dr,6); // 1/r^12 - 1/r^6, lennard-jones up to a factor 1/4
      }
    }
  }

  return 4.0*ene; //add the factor just once at cycle end
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   double r, gdir;
   double deltaV;
   ofstream Gofr, Gave, Gave_err, Epot, Pres;
   const int wd=12;
    ...
    Gave_err.open("output.gave_err.0",ios::app);
	...
	r = (ibin+1)*bin_size;
	deltaV = 4.*M_PI/3. *(pow(r+bin_size,3) -pow(r,3));
	stima_g[ibin] = blk_av[igofr+ibin]/blk_norm/rho/deltaV;
	glob_av[igofr+ibin] += stima_g[ibin];
	glob_av2[igofr+ibin] += stima_g[ibin];
	err_g[ibin] = Error(glob_av[igofr+ibin],glob_av2[igofr+ibin],iblk);
    }
...
//g(r)
    if( iblk == nblk){
	for(int ibin= 0; ibin<nbins;ibin++){
		Gave << stima_g[ibin] << endl;
		Gave_err << err_g[ibin] << endl;
	}
    }
